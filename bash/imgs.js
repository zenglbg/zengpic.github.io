...
function consoleTree(tree, floor = 1, str = "", adder = "", isLast = false) {
  ...
  for (let i = 0; i < tree.length; i++) {
    ...
    //   !tree[i].children
    // ) {
    //   console.log(path.join(basepath, tree[i].name));
    //   if (fs.lstatSync(path.join(basepath, tree[i].name)).isFile()) {
    //     fileTree += "\n" + "└" + str + tree[i].name;
    //   } else {
    //     fileTree += "\n" + "# " + str + tree[i].name;
    //   }
    // }
      ...
      if (fs.lstatSync(path.join(basepath, tree[i].name)).isFile()) {
        if (/(mp4)/gi.test(tree[i].name)) {
          fileTree += `\n [${
            tree[i].name.split("/").slice(-1)[0]
          }](/${encodeURIComponent(tree[i].name)}) \n`;
        } else {
          fileTree += `\n ![${
            tree[i].name.split("/").slice(-1)[0]
          }](/${encodeURIComponent(tree[i].name)}) \n`;
